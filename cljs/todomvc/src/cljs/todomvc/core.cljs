(ns todomvc.core
  (:require [cljs.pprint :as pp]
            ...
            [tiltontec.tag.gen :refer-macros [on-evt on-evt! section header h1 input footer p a span label ul li div button]
             :refer [dom-tag]]
            [todomvc.todo :refer [gTodo gItems-raw gTodo-lookup TODO_LS_PREFIX make-todo td-to-map td-title td-completed
              ...
              [(section (:class "todoapp" :par :top)
                        (header (:class "header")
                                ...
                                (input (:class "new-todo" :autofocus true
                                         ...
                                         :onkeypress (tiltontec.tag.gen/on-evt! todomvc.core.todo-process-on-enter))))
                        ...
                        (section (:class "main"
                                 ...
                                 (label (:for "toggle-all"
                                          :onclick (on-evt! todomvc.core/td-completed-toggle-all))
      ...
      (li (:todo td
          ...
          (div (:class "view")
               (input (:class "toggle" :input-type "checkbox"
                        ...
                        :onclick (on-evt! todomvc.todo/td-toggle-completed (td-id td))))

               (label (:ondblclick (on-evt! todomvc.core/todo-start-editing))
                      (td-title td))

               (button (:class "destroy"
                         :onclick (on-evt! todomvc.todo/td-delete-by-key (td-id td)))))

          (input (:class "edit"
                   :value (c?n (td-title td))
                   :onblur (on-evt! todomvc.core/todo-edit (td-id td))
                   :onkeydown (on-evt! todomvc.core/todo-edit (td-id td))))))

(defn ^:export mk-dashboard []
      (footer (:class "footer"
                :hidden  (c? (zero? (count (gTodo-items)))))
        ...
        (button (:class "clear-completed"
                  ...
                  :onclick (on-evt! todomvc.todo/td-clear-completed))
          "Clear completed")))
