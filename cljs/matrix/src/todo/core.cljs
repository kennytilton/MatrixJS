...
(declare todo-start-editing todo-edit todo-process-on-enter)
    ...
    [(section (:class "todoapp" :par :top)
        (header (:class "header")
          ...
          (input (:class "new-todo" :autofocus true
                  ...
                  :onkeypress (on-event todo.core.todo-process-on-enter))))
        ...
        (section (:class "main"
          ...
          (label (:for "toggle-all"
                  :onclick (on-event todo.core.td-completed-toggle-all))
  ...
  (li (:todo td
    ...
    (div (:class "view")
         (input (:class "toggle" :input-type "checkbox"
                  ...
                  :onclick (on-event todo.todo.td-toggle-completed (td-id td))))

         (label (:ondblclick (on-event todo.core.todo-start-editing))
                (td-title td))

         (button (:class "destroy"
                   :onclick (on-event todo.todo.td-delete-by-key (td-id td)))))

    (input (:class "edit"
             :value (c?n (td-title td))
             :onblur (on-event todo.core.todo-edit (td-id td))
             :onkeydown (on-event todo.core.todo-edit (td-id td))))))
...
(defn mk-dashboard []
  (footer (:class "footer"
    ...
    (button (:class "clear-completed"
              ...
              :onclick (on-event todo.todo.td-clear-completed))
            "Clear completed")))

;;; --- event handlers to support the above ------------------------------------

(defn ^:export todo-process-on-enter [e]
	(when (= (.-key e) "Enter")
...
(defn ^:export todo-start-editing [e]
...
(defn ^:export todo-edit [e td-key]
...
(defn ^:export td-completed-toggle-all [event]
